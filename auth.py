...
class Auth:
    ...
    def __init__(self, username, password):
        RiotClient = requests.get('https://valorant-api.com/v1/version')
        RiotClientJson = RiotClient.json()
        RiotClientBuild = RiotClientJson['data']['riotClientBuild']
        self.username = username
        self.password = password
        self.RiotClientBuild = RiotClientBuild
        self.session = requests.Session()
        self.session.headers = OrderedDict({"User-Agent": "RiotClient/" + self.RiotClientBuild + "%s (Windows;10;;Professional, x64)","Accept-Language": "en-US,en;q=0.9","Accept": "application/json, text/plain, */*"})
        ...
        self.base_headers = {'User-Agent': "RiotClient/" + self.RiotClientBuild + "%s (Windows;10;;Professional, x64)",'Authorization': f'Bearer {self.access_token}',}
